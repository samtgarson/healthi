function tabChange(tab, newIcon, newText) {
    $(tab.$el).animate({
        ...
        tab.icon = newIcon, tab.text = newText, $(tab.$el).css("textIndent", "-50px").promise().done(function() {
            $(tab.$el).animate({
                textIndent: "0px"
            }, 150, "easeInQuad");
        });
...
function goalFunc() {
    app.$.View.$.tabSkip.reset(), app.$.View.$.tabInfo.reset(), app.$.View.skipping = !1;
...
function loginClick() {
    ...
    $.ajax({
        ...
        storage.set(json), $this.$parent.loadData(), $this.$parent.changePage("home");
    });
}
...
Vue.component("tab-done", {
    ...
    methods: {
        click: function() {
            app.$.View.$.cardref[app.$.View.currentGoal].complete(), app.$.View.$.tabInfo.reset(), 
            app.$.View.$.tabSkip.reset();
        },
        change: tabChange,
        reset: function() {
            "cancel" == this.text && this.change(this, "t", "done");
        }
    }
    ...
    methods: {
        click: function() {
            app.$.View.$.tabInfo.reset(), "skip" == this.text ? (this.change(this, "b", "cancel"), 
            app.$.View.skipping = !0) : this.reset();
        },
        change: tabChange,
        reset: function() {
            app.$.View.skipping = !1, "cancel" == this.text && this.change(this, "s", "skip");
        }
    }
    ...
    methods: {
        click: function() {
            app.$.View.$.tabSkip.reset(), this.change(this, "cancel" == this.text ? "i" : "b", "cancel" == this.text ? "info" : "cancel"), 
            app.$.View.$.cardref[app.$.View.currentGoal].flip();
        },
        change: tabChange,
        reset: function() {
            app.$.View.$.cardref[app.$.View.currentGoal].flip(), "cancel" == this.text && this.change(this, "i", "info");
        }
    }
    ...
    methods: {
        ...
        change: tabChange,
        reset: function() {
            "cancel" == this.text && this.change(this, "c", "comment");
        }
    }
    ...
    methods: {
        skip: function() {
            app.$.View.$.cardref[app.$.View.currentGoal].skip(), app.$.View.$.tabSkip.reset();
        }
    }
}), Vue.component("card", {
    template: "#cardTpl",
    ...
    ready: function() {
        var e = this.$el;
        setTimeout(function() {
            $(e).children("div.subject").animate({
                opacity: 1,
                marginTop: "0px"
            }, 200, "easeOutQuint");
        }, 500), this.$on("congratsFade", function(index, callback) {
            index == this.index && $(this.$el).children("div.subject").animate({
                marginTop: "50px",
                opacity: 0
            }, 200, "easeInQuint").promise().done(function() {
